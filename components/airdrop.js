...
export default function AirDrop({data}) {
    const { connected, walletAddress, handleConnect, handleDisconnect,wallet, chainId,waitForTransaction } = useWeb3()
        const { getTotalStaked2, compound,getPoolInfo,claim,claimAirdrop,getAmount,getUserInfo, getClaimStatus,getTokenPerBlock,getRewardPerYear,approveNewStake, getPending, smgTokenPrice } = useContracts()
    ...
    const onClickAction = async (address,decimal) => {
        if(!connected)
            handleConnect();
                ...
                var a = await getAmount(contract,decimal);
                var status = await getClaimStatus(contract);
    ...
    const handleClaim = async (decimal,address) => {
        ...
        const bigAmount = utils.parseUnits(camount.toString(), decimal)
            claimAirdrop(contract,bigAmount).then(async (result) => {
    ...
    return (
     ...
     <Card key={`airdrop-${data.address}`} className="p-3">
         ...
         {!connected ? (
                     ...
                     ( 
                         <div className="btn btn-block v-btn--rounded mt-4 btn-primary" onClick={() => switchNetwork(addresses.networkID)}>Switch To BSC MainNet</div>
