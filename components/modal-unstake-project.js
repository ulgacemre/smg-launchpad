...
const ModalUnStake = ({isStake = true, className = '', show = false, onClose = () => {}, ...props}) => {
    const { getBalance, smgTokenPrice ,deposit2,withdraw2,emergencyWithdraw,getPoolInfo, getUserInfo} = useContracts()
    ...
    const [stakeContract, setStakeContract] = useState(new Contract(address || props.addrees || "0xC090f7D3368a36b20b2B2553386f4E5E184cecb9", abis.NewStake))  
    ...
    return (
        <Modal title={isStake ? "Stake DEUX tokens" : "Unstake DEUX tokens"} className={className} show={show} onClose={onClose} {...props}>
            ...
            <div className="py-2  mt-3">
               Stake Balance : {utils.commify(isStake ? balance : userInfo?.amount)} DEUX
            </div>
